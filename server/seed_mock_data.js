...
// Mock Data (Copied from client/src/lib/mockData.js)
const mockData = {
    categories: [
...
async function seedSellers() {
    ...
    for (const seller of mockData.sellers) {
        const email = `${seller.username}@example.com`;
        const phone = `555${String(seller.user_id).padStart(7, '0')}`;
        const password = "123456"; // Default password
        const registration_date = new Date().toISOString().slice(0, 10);

        // Check if user exists
        const existing = await query("SELECT user_id FROM users WHERE user_id = ?", [seller.user_id]);

        if (existing.length === 0) {
            const sql = `
                INSERT INTO users 
                (user_id, username, password, first_name, last_name, phone_number, email, registration_date, rating, loyalty_points, role)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'seller')
            `;
            await query(sql, [
                seller.user_id,
            ...
            console.log(`Seller ${seller.username} added.`);
        } else {
            console.log(`Seller ${seller.username} already exists.`);
        }
    }
}
...
async function seedProducts() {
    ...
    for (const product of mockData.products) {
        ...
        const existing = await query("SELECT product_id FROM product WHERE product_id = ?", [product.product_id]);

        if (existing.length === 0) {
            const sql = `
                INSERT INTO product 
                (product_id, seller_id, name, description, price, upload_date, photo, quantity, is_available)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
            await query(sql, [
                product.product_id,
